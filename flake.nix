{
  ...
  inputs = {
    ...
    pikvm-mcp-server = {
      url = "github:dvaerum/pikvm_mcp_server/b5fdc31e16525f196282436a0fa7ca797eea2e5e";
      inputs.nixpkgs.follows = "nixpkgs";
    };
  };
  ...
  outputs =
    ...
    in
    {
      ...
      # pinned disko PR #1190 + host binfmt for aarch64). (also --board
      # pikvm01 for the iPad-default kiosk, it-03400 for the hardware-
      # verification appliance, or zero2w — NOT universal, which has no disko
      # config and is built as a single image file instead, see README)
      apps = forAllSystems (
        system:
        let
          pkgs = pkgsFor system;
            ...
            text = ''
              ...
              while [ $# -gt 0 ]; do
                case "$1" in
                  ...
                  -h|--help)
                    echo "usage: nix run .#install-sd -- [--board rpi4|pikvm01|it-03400|zero2w] /dev/DISK"; exit 0 ;;
                  *) device="$1"; shift ;;
                esac
              done
              if [ -z "$device" ]; then
                echo "error: no target disk given" >&2
                echo "usage: nix run .#install-sd -- [--board rpi4|pikvm01|it-03400|zero2w] /dev/DISK" >&2
                exit 1
              fi
